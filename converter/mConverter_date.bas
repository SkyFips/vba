Attribute VB_Name = "mConverter_date"
Option Explicit
' ##############################################################################\
...
Public Function parseIso8601(isoString As String, Optional asLocalDate As Boolean = False) As Date
  ...
  With matches(0)
    parseIso8601 = VBA.DateSerial(VBA.CInt(.submatches(0)), VBA.CInt(.submatches(1)), VBA.CInt(.submatches(2))) + _
                   VBA.TimeSerial(.submatches(3), .submatches(4), .submatches(5))
    'logger.log INFO, CStr(parseIso8601)
    Select Case Left(.submatches(6), 1)
    Case "+"
      parseIso8601 = parseIso8601 - TimeSerial(.submatches(7), .submatches(8), 0)
    Case "-"
      parseIso8601 = parseIso8601 + TimeSerial(.submatches(7), .submatches(8), 0)
    End Select
  End With
  If asLocalDate Then parseIso8601 = convertToLocalDate(parseIso8601)
End Function

Public Function toIso8601(localDate As Date) As String
  ...
  m = i.bias Mod 60
  If h < 0 Or m < 0 Then
    toIso8601 = VBA.Format$(localDate, "yyyy-mm-ddTHH:mm:ss.000") & "+" & VBA.Format$(Abs(h), "00") & ":" & VBA.Format$(Abs(m), "00")
  ElseIf h > 0 Or m > 0 Then
    toIso8601 = VBA.Format$(localDate, "yyyy-mm-ddTHH:mm:ss.000") & "-" & VBA.Format$(h, "00") & ":" & VBA.Format$(m, "00")
  Else
    toIso8601 = VBA.Format$(localDate, "yyyy-mm-ddTHH:mm:ss.000Z")
  End If
End Function
